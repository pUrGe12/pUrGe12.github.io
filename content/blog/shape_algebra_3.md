...
The `--max-states` bail-out is there because the state count is what actually kills you, not the shape count. Each mirror and pin multiplies the number of *states* sitting on a shape even though the geometry stays put, so `len(cur)` runs well ahead of `len(reached[n])`.

3. Then we extract the physical geometry from every shape, by reversing every step of the integer generation.
4. Enumerates over 1 to 10.

Extracting the geometry is just `shape_key`, which throws away the latch, the pin flag and both summaries and keeps the $(W, H, \text{mask})$ triple. Two states that draw the same picture but remember different history collapse to the same key here:
...
To know whether that set is actually *complete*, we need something to compare it against, so the script grows every fixed polyomino of size $n$ independently, by brute force, and keys them the same way:
...
The set difference between those two is the answer, and any shape left over gets printed so you can look at it:
...
Running it gives exactly what the first two posts claimed, including the nineteen tetrominoes at $n = 4$:

```
$ python3 without_sub.py sweep 6
BFS (with P):
  n= 1:         1 shapes,          4 states,      0s
  n= 2:         2 shapes,         16 states,      0s
  n= 3:         6 shapes,         76 states,      0s
  n= 4:        19 shapes,        372 states,      0s
  n= 5:        63 shapes,       1770 states,      0s
  n= 6:       216 shapes,       8144 states,      0s

  n  reachable      total  missing
  1          1          1        0
  2          2          2        0
  3          6          6        0
  4         19         19        0
...
## Going past the sweep

The sweep is the honest way to answer "can we make *everything* of size $n$", but the state count grows fast enough that it stops being an option somewhere around $n = 12$. If the question is only about one particular shape, we can prune much harder.

The observation is that every intermediate shape in a construction that is purely additive has to be a connected sub-shape of the target, or of one of the target's mirror images. So we precompute that set of allowed keys and refuse to walk anywhere outside it:
...
Then the search itself is the same BFS, except mirrors, pins and additions are all live at once, and every candidate is filtered through `allowed` before it goes on the queue:
...
This is the mode that produced the $N = 11$ failures. You hand it a shape drawn as text and it tells you yes or no:

```
$ python3 without_sub.py check shapes.txt
n=11  reachable=False  (with P)
XXXXX
X...X
XX.XX
...
Those are the two interior-reachability shapes from the previous posts, and they both come back `False` here, which is the expected answer for this file: it is the purely additive fragment, and closing that gap is exactly what subtraction was introduced for in the last post.

And `--nop` turns pinning off everywhere, which is a quick way to confirm that a given shape genuinely needs the pin rather than just happening to use it.

One caveat on the pruning: it is only valid because this file has no subtraction. The moment a construction is allowed to remove cells, an intermediate shape can be *larger* than the target and the sub-shape filter would throw away a real path. That is the main reason this particular script stops at the additive fragment.
