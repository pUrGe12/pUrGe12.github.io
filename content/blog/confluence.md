...
## A note on multiple spellings

A shape having several expressions is not in itself a problem --- $1$, $2/2$ and $e^0$ all denote the same number. It matters only if the notation is meant to *count* shapes, since then the number of expressions has to be corrected for how many spell the same thing.

That correction is a known one. Symmetric shapes are the source of the duplication: a shape equal to its own mirror image is spelled by both $F$ and $F^M$. This is the distinction between *fixed*, *one-sided* and *free* polyominoes --- counted with all orientations distinct, up to rotation, and up to rotation and reflection respectively:
...
The three columns are related by Burnside's lemma over the eight symmetries of the square. Checking which column the expression counts match is a quick test of whether the notation behaves as intended.
