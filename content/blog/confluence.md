...
$$
\left(\left(\left(\left(\left(\left(\left(A_C^{(2)}+1_R\right)^M\right)_C+1_R\right)_P+1_C\right)_R+1_R\right)^M\right)^M_C+1_C\right)+1_R
$$

(where a bare $(\cdot)^M$ means $(\cdot)^{M}_{R}$)
...
And that's the target. This shape is symmetric about its diagonal, so tacking a final $(\\,\cdot\\,)^{M}$ on the end would just give the same shape turned by 90°.
...
$$
\left(\left(\left(\left(\left(A_C^{(3)} + 1_R\right) + 1_R\right)^M\right)_C + 1_R\right)+1_C\right)+1_C
$$
...
Link for the 2nd part: [part 2](https://purge12.github.io/blog/shape-algebra-2/)
