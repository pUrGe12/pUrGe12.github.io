...
{{ figure(src="assets/BaseBase_case_non_rotated.png", alt="The base case provided", caption="The base case provided with no knowledge of orientation or colours") }}

Since the statements are true, we have `P3` telling us that the two Rooks (**R**) are of different colours and `P4` telling us that no piece is on a square that matches its own colour. In the board, no matter what orientation we pick, the two rooks will always be on the same coloured square. So, according to `P4` they should be of the same colours (if they're both in the **Black square**, they they both should be **White** and vice-versa). But that contradicts with `P3`!
...
{{ figure(src="assets/non_rotated_base_case.png", alt="The base case with annotations", caption="The base case with coloured annotations following the rules") }}

The thinking behind this setup is as follows:

1. We have two Knight, one must be White and one must be Black
2. Only one bishop can be protected by a Knight, so both of those must be the same colour (White here).
...
4. The only pieces that could possible not have moved are the White Pawns in the 2nd row from the bottom. So, at least one of them should be White (I've used both White but as we'll see it won't make a difference)
...
If we switch colours, make every White piece Black and so on, we'll see that it breaks `~P5` since the White Knight now won't be threatened by any Pawn.

Hence, this orientation will not work.

### Rotate CW by 90 degrees

{{ figure(src="assets/90_degrees_CW.png", alt="90 degrees CW rotated case", caption="The 90 degrees CW rotated case with coloured annotations following the rules") }}

We follow the same procedure as before, and everything checks out almost exactly right. Almost. Note that we applied `~P2` here as well, which forced the two Rooks to be the same colour and White. This tipped the number of White pieces to 8 and Black pieces to 5. Since two Bishops are still un-coloured, and we have 4 in the game with no promotions, one of them must be White and one must be Black. So the total number of `White pieces > Black pieces + 1`. Hence `~P2` was satisfied.

Anyway, the issue here is again `~P5`. In this case, the diagram shows the White Knight in the rightmost column not being threatened by any Pawn.

This time we CANNOT flip the colours, because of `~P10`, since the only piece which could have possibly stated in its own place the entire game is the White pawn in row 2 from the bottom. So, it must be White. And hence we have a **contradiction**.

### Rotate CW by 180 degrees

{{ figure(src="assets/180_degrees_CW.png", alt="180 degrees CW rotated case", caption="The 180 degrees CW rotated case with coloured annotations following the rules") }}
...
{{ figure(src="assets/270_degrees_CW.png", alt="270 degrees CW rotated case", caption="The 270 degrees CW rotated case with coloured annotations following the rules") }}

Following the same procedure here we easily get away without any contradictions. We're forced to choose the two Rooks to be Black due to `~P2`, and we still don't have a confirmation on which bishop is what colour of the two pink ones.
...
Since we have two bishops which can be assigned any colour (albeit one black and one white), let's try out both permutations and see what we get.
